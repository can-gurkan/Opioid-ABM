...
to-report Rescue-event?
  ...
  report ifelse-value random-float 1 < default-death-rate * (1 - treatment-factor) [false] [true]
...
;  ]
  ...
  if care-seeking? = FALSE and in-treatment? = FALSE [
    ...
    if intervention-scenario = 5 [set linkage-rate 100]
...
to potentially-drop-out
  ...
  if intervention-scenario = 6 [set drop-out-chance 0]
...
<experiments>
  ...
  <experiment name="max-treatment-additional-exp" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="365"/>
    <metric>count people</metric>
